...
pub struct InitializeSwap<'info> {
    ...
    /// CHECK: This is the swap escrow account, verified in the program logic.
    #[account(init,
    payer = seller,
...
pub fn handle(
    ...
    verify_spl_token(&ctx.accounts.token_mint.to_account_info(), &ctx.accounts.seller_token_account.to_account_info())?;
